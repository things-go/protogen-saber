...
func runProtoGen(gen *protogen.Plugin) error {
	...
	if args.Merge {
		...
		sources = make([]Source, 0, len(gen.Files))
	}
	...
	for _, f := range gen.Files {
		...
		if args.Merge {
			sources = append(sources, Source{
				f.Desc.Path(),
				f.Proto.GetOptions().GetDeprecated(),
			})
		...
		for _, tb := range tables {
			...
			setGeneratedFileHeader(g, gen, []Source{
...
func setGeneratedFileHeader(g *protogen.GeneratedFile, gen *protogen.Plugin, sources []Source) {
