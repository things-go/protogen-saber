...
{{- if or (eq .Code 400) (eq .Code 500)}}
	return errors.NewWithStatusCode({{.Status}}, {{.Code}}, "{{.Message}}", detail)
{{- else}}
	if len(message) > 0 {
...
{{- if or (eq .Code 400) (eq .Code 500)}}
	 return errors.NewWithStatusCode({{.Status}}, {{.Code}}, "{{.Message}}", fmt.Sprintf(format, args...))
{{- else}}
	 return Err{{.CamelValue}}w(WithMessage(fmt.Sprintf(format, args...)))
{{- end}}
}
func Err{{.CamelValue}}w(opt ...Option) *errors.Error {
	e := errors.NewWithStatusCode({{.Status}}, {{.Code}}, "{{.Message}}", {{$enumName}}_{{.Value}}.String())
	_apply(e, opt...)
	return e
}
{{- end }}
